...
from .freq_pixel_loss import find_high_freq
from .freq_pixel_loss import get_gaussian_kernel
...
class ResidualDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_input, x, t, x_input_condition=0, x_self_cond=None):
        preds = self.model_predictions(x_input, x, t, x_input_condition, x_self_cond)
    ...
    def sample(self, x_input=0, batch_size=16, last=True):
        ...
        if self.condition:
            if self.input_condition and self.input_condition_mask:
                ...
                x_input = normalize_to_neg_one_to_one(x_input) # 归一化，调整至[-1,1]
    ...
    def p_losses(self, imgs, t, noise=None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_res = x_input - x_start # 原始代码

        #######################换成高频信息相减试试#########################################
        # gauss_kernel = get_gaussian_kernel(21).cuda()
        # x_input_hfreq = find_high_freq(x_input,gauss_kernel)
        # x_start_hfreq = find_high_freq(x_start,gauss_kernel)
        # x_res = x_input_hfreq - x_start_hfreq
        #######################换成高频信息相减试试#########################################
...
class Trainer(object):
    ...
    def sample(self, milestone, last=True, FID=False):
        ...
        with torch.no_grad():
            ...
            if self.condition_type == 0:
                ...
                show_x_input_sample = x_input_sample
                x_input_sample = x_input_sample[1:]
            meiyong = self.ema.ema_model.sample(x_input_sample, batch_size=batches, last=last)
            all_images_list = show_x_input_sample + list(self.ema.ema_model.sample(x_input_sample, batch_size=batches, last=last))
    ...
    def test(self, x_input_sample,sample=False, last=True, FID=False):
        x_input_sample=[x_input_sample]
        # print("test start")
        if self.condition:
            self.ema.ema_model.eval()
            # loader = DataLoader(
            ...
            with torch.no_grad():
                ...
                if sample:
                    # 没走这步
                    all_images_list = x_input_sample + \
            ...
            # utils.save_image(all_images, r'E:\Pythonfiles\LPTN-main\hahaahha\pre_lq\0.png')
            return all_images
